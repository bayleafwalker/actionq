...
| 5.5a | **Done.** `w4r-reissued-identity-cannot-inherit`'s ownership half now has a database-backed test (`tests/test_federation_ownership_authority.py::test_a_reissued_epoch_cannot_mutate_the_prior_epochs_resource`); the shape-refusal half was split out into its own falsifier, `w4r-unminted-principal-refused`, still resting on `tests/test_federation_catalog_contract.py::test_an_unminted_principal_id_is_refused_before_any_command`. Pathway: R1.1.3. | rescope §6; W4 build | actionq | — |
