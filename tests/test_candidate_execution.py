...
def test_verification_runs_only_registered_frozen_command_in_fresh_bundle(tmp_path):
    ...
    result_ref = verify_candidate(store, spec_ref=spec_ref)
...
def test_wave_integration_preserves_frozen_member_ordinal_order(tmp_path):
    ...
    def passed(candidate_ref: str, marker: str) -> str:
        spec_ref = "artifact:sha256:" + marker * 64
        return _put_contract(store, {"contract_id": CANDIDATE_VERIFICATION_RESULT_V1,
    ...
    first_result, second_result = passed(first_ref, "a"), passed(second_ref, "b")
    integration = {"contract_id": CANDIDATE_INTEGRATION_SPEC_V1,
        "request_ref": "artifact:sha256:" + "c" * 64, "request_digest": "sha256:" + "c" * 64,
    ...
    result_ref = integrate_wave(store, spec_ref=integration_ref, author_name="ActionQ", author_email="actionq@example.invalid", commit_timestamp="2026-08-01T00:00:00Z")
...
def test_stacked_integration_returns_the_existing_tip_without_a_synthetic_bundle(tmp_path):
    ...
    first_result, second_result = passed(first_ref, "e"), passed(second_ref, "f")
    ...
    result_ref = integrate_wave(store, spec_ref=_put_contract(store, spec), author_name="ActionQ", author_email="actionq@example.invalid", commit_timestamp="2026-08-01T00:00:00Z")
    result = resolve_exact_contract(store, result_ref, CANDIDATE_INTEGRATION_RESULT_V1)
    assert result["outcome"] == "integrated"
